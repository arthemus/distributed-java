...
public class PageRankTest extends TestCase {
	...
	private static JavaSparkContext getSparkContext(final int nCores) {
		...
		final SparkConf conf = new SparkConf()
			.setAppName("io.distributed.PageRank")
	...
	private static Website generateWebsite(final int i,
										   final int nNodes,
	...
	private static JavaPairRDD<Integer, Website> generateGraphRDD(final int nNodes,
		...
		return context.parallelize(nodes).mapToPair(i -> new Tuple2<>(i, generateWebsite(i, nNodes, minEdgesPerNode, maxEdgesPerNode, edgeConfig)));
	}
	...
	public void testUniformOneThousand() {
	...
	private static void testDriver(final int nNodes,
								   final int minEdgesPerNode,
								   final int maxEdgesPerNode,
		...
		for (int i = 0; i < parResult.size(); i++) {
			final var id = nodesArr[i].getId();
			assertTrue(keyed.containsKey(id));
			final double delta = Math.abs(ranksArr[i] - keyed.get(id));
		...
		final String msg = "Expected at least " + expectedSpeedup + "x speedup, but only saw "
			+ speedup + "x. Sequential time = " + singleElapsed + " ms, parallel time = " + parElapsed + " ms";
