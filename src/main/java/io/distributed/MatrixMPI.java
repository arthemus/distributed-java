...
public final class MatrixMPI {
	 ...
	 * @throws MPI.MPIException On MPI error. It is not expected that your
	 *                          implementation should throw any MPI errors during
	 *                          normal operation.
	 */
	public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c, final MPI mpi) throws MPI.MPIException {
		...
		if (myrank == 0) {
			...
			final int offset = startRow * c.getNCols();
			final int count = (endRow - startRow) * c.getNCols();
			mpi.MPI_Send(c.getValues(), offset, count, 0, myrank, mpi.MPI_COMM_WORLD);
		}
	}
}
