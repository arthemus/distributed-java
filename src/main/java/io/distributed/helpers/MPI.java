package io.distributed.helpers;
...
public final class MPI {
	...
	private interface MPILib extends Library {
		MPILib INSTANCE = Native.loadLibrary("mpi", MPILib.class);
	 ...
	 * @throws MPIException On MPI error
	 */
	public void MPI_Bcast(final double[] buf, final int offset, final int count, final int root, final MPI_Comm comm) throws MPIException {
	 ...
	 * @throws MPIException On MPI error
	 */
	public void MPI_Send(final int[] buf, final int offset, final int count, final int dst, final int tag, final MPI_Comm comm) throws MPIException {
	 ...
	 * @throws MPIException On MPI error
	 */
	public void MPI_Send(final double[] buf, final int offset, final int count, final int dst, final int tag, final MPI_Comm comm) throws MPIException {
		final DoubleBuffer wrapper = DoubleBuffer.wrap(buf, offset, count);
	 ...
	 * @throws MPIException On MPI error
	 */
	public void MPI_Recv(final int[] buf, final int offset, final int count, final int src, final int tag, final MPI_Comm comm) throws MPIException {
		final IntBuffer wrapper = IntBuffer.wrap(buf, offset, count);
		final int err = MPILib.INSTANCE.MPI_Recv(wrapper, count, MPI_INTEGER.datatype, src, tag, comm.comm, MPI_STATUS_IGNORE.status);
		if (err != 0) {
			throw new MPIException(err);
		}
	}
	 ...
	 * @throws MPIException On MPI error
	 */
	public void MPI_Recv(final double[] buf, final int offset, final int count, final int src, final int tag, final MPI_Comm comm) throws MPIException {
		final DoubleBuffer wrapper = DoubleBuffer.wrap(buf, offset, count);
		final int err = MPILib.INSTANCE.MPI_Recv(wrapper, count, MPI_DOUBLE.datatype, src, tag, comm.comm, MPI_STATUS_IGNORE.status);
		if (err != 0) {
			throw new MPIException(err);
		}
	}
	 ...
	 * @throws MPIException On MPI error
	 */
	public MPI_Request MPI_Irecv(final double[] buf, final int offset, final int count, final int src, final int tag, final MPI_Comm comm) throws MPIException {
		final Recv_MPI_Request request = new Recv_MPI_Request(count * 8L, buf, offset, count);
		final int err = MPILib.INSTANCE.MPI_Irecv(request.buf, count, MPI_DOUBLE.datatype, src, tag, comm.comm, request.request);
		if (err != 0) {
			throw new MPIException(err);
		}
		return request;
	 ...
	 * @throws MPIException On MPI error
	 */
	public MPI_Request MPI_Isend(final double[] buf, final int offset, final int count, final int dst, final int tag, final MPI_Comm comm) throws MPIException {
		final Send_MPI_Request request = new Send_MPI_Request(count * 8L);
		request.buf.write(0, buf, offset, count);
		final int err = MPILib.INSTANCE.MPI_Isend(request.buf, count, MPI_DOUBLE.datatype, dst, tag, comm.comm, request.request);
		if (err != 0) {
			throw new MPIException(err);
		}
		return request;
	...
	public abstract static class MPI_Request {
		 ...
		 */
		protected MPI_Request(final long size) {
			this.request = new Memory(8);
			this.buf = new Memory(size);
		}
