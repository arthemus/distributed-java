...
public class PCDPFilesystem {
	...
	public void addFile(PCDPPath path, String contents) {
		...
		final var pathComponent = path.getComponent(0);
		if (!pathComponent.equals(root.getName()))
			throw new RuntimeException();

		int componentIndex = 1;
		PCDPFolder curr = root;
		while (componentIndex < path.getNComponents()) {
			final String component = path.getComponent(componentIndex++);
			final PCDPFilesystemComponent next = curr.getChild(component);
			if (componentIndex < path.getNComponents()) {
				// Haven't reached bottom of path yet so must be folder
				if (next == null) {
					final PCDPFolder newFolder = new PCDPFolder(component);
				...
				assert next == null;
				final PCDPFile newFile = new PCDPFile(component, contents);
	...
	public String readFile(PCDPPath path) {
		...
		while (componentIndex < path.getNComponents()) {
			...
			if (!(curr instanceof PCDPFolder)) {
				return null;
			}
			curr = ((PCDPFolder) curr).getChild(nextComponent);
		}

		if (!(curr instanceof PCDPFile)) {
			return null;
		} else {
